...
export class MdSelectMenu extends HTMLElement {
	...
	getContent() {
		return /* HTML */ `
			...
			<output></output>
			<span id="down" class="material-symbols-outlined">arrow_drop_down</span>
			<span id="up" class="material-symbols-outlined">arrow_drop_up</span>
		`;
	}
	...
	get _valueIndex() {
		const value = this.value;
		const optionsMenu = this.optionsMenu;
		return optionsMenu === null ? -1 : optionsMenu.opciones.findIndex((opcion) => opcion.dataset.value === value);
	}
	...
	clicExterno(evt) {
		...
		if (this.classList.contains("open") && target instanceof HTMLElement && parentElement !== null && !parentElement.contains(target) && optionsMenu !== null && !optionsMenu.contains(target)) {
			this._cierra();
		}
	}
